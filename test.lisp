...
(deftest :yason "other-symbols-for-bool"
  (alexandria:with-gensyms (a-true a-false a-null)
    (let ((yason:*true*  a-true)
          (yason:*false* a-false)
          (yason:*null*  a-null))
      (test-equal a-true  (yason:parse "true")  :test #'eq)
      (test-equal a-false (yason:parse "false") :test #'eq)
      (test-equal a-null  (yason:parse "null")  :test #'eq))
    (let ((yason:*true*  (list a-true  2))
          (yason:*false* (list a-false 3))
          (yason:*null*  (list a-null  4)))
      (test-equal a-true  (yason:parse "true")  :test #'eq)
      (test-equal a-false (yason:parse "false") :test #'eq)
      (test-equal a-null  (yason:parse "null")  :test #'eq))
    (labels ((encode-to-string (data)
      ...
      (let ((yason:*true* a-true)
            (yason:*false* a-false)
            (yason:*null* a-null))
        (test-equal "true"  (encode-to-string a-true))
        (test-equal "false" (encode-to-string a-false))
        (test-equal "null"  (encode-to-string a-null)))
      (let ((yason:*true*  (list a-true  2))
            (yason:*false* (list a-false 3))
            (yason:*null*  (list a-null  4)))
        (test-equal "true"  (encode-to-string a-true))
        (test-equal "false" (encode-to-string a-false))
        (test-equal "null"  (encode-to-string a-null)))
      (let ((yason:*true*  (list 2 a-true ))
            (yason:*false* (list 3 a-false))
            (yason:*null*  (list 4 a-null )))
        (test-equal "true"  (encode-to-string a-true))
        (test-equal "false" (encode-to-string a-false))
        (test-equal "null"  (encode-to-string a-null))) )))
